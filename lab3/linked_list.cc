...
void delete_last_element(Node*& head){
  ...
  // Guaranteed to be at least 2 nodes, iterate to 2nd last node
  Node* node = head; 
  while(node->next->next != NULL){
     node = node->next;
  }
  // And from there, delete last node
  node->next = NULL;
}
...
void remove(Node*& head, int oldKey) {
  ...
  if (head == NULL) return;
  // Check if head contains old key
  if (head->key == oldKey){
     head = NULL;
     return;
  }
...
Node* interleave(Node* list1, Node* list2){
   ...
   // Check if both linked lists are empty. If so, return null
   if (node1 == NULL && node2 == NULL)
      return NULL;

   // Control which head to start from, incase list1 is null
   if(node1 != NULL){
      currNode->key = node1->key;
      node1 = node1->next;
      head = currNode;
   }
   else if(node2 != NULL){
      currNode->key = node2->key;
      node2 = node2->next;
      head = currNode;
   }

   // Linking and interleaving step -- iterate through the lists and add each 1 by 1 to new linked list
