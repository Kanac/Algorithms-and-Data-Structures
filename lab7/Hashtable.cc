...
Hashtable::Hashtable(int size) {
	...
	int prime = nextPrime( size ); 
        _secondHashPrime = prevPrime(size);
	_size = size;
	if (prime != size) { 
...
int Hashtable::secondHash(int k){
	return _secondHashPrime - (k % _secondHashPrime);
}
...
void Hashtable::qinsert(int k) {
    ...
    while (probes <= _size){
       ...
       index = (indexSave+probes*probes) % _size; 
       ++probes; 
    }
...
void Hashtable::dinsert(int k) {
    ...
    do{
       ...
       index = (indexSave+jump*probes) % _size;
       ++probes; 
    } while ( index != indexSave );
...
int Hashtable::prevPrime( int n ) { 
    for (int ii = n - 1; ii > 0; ii-- ){ 
      if ( isPrime( ii ) ) return (ii); 
    }
    assert( false ); // logic error 
}
