...
void printHeap(int *heap, int size, int node=0, int d=0) {
  ...
  // Question 2
	...
	d++;
	printHeap(heap, size, 2 * node + 2, d);
	cout << std::string(d*2, ' ') << heap[node] << std::endl;
	printHeap(heap, size, 2 * node + 1, d);
...
void remove(int* heap, int key, int & size) {
	...
	// Create new heap and size where only values that don't match key are taken
	...
	// Heapify new array and set reference size
	size = newSize;
	heapify(newHeap, size);

	// Copy new values over to our actual heap
...
int* mergeHeap(int* heap1, int* heap2, int size1, int size2) {
  // TODO: replace the following line with your code for Question 4
	...
	// Copy 2nd heap over from next position of 1st heap
